#include "atlas.h"

#include "utils/graphics_util.h"

void Atlas::LoadFromFile(std::string path, int num) {
    img_list_.clear();
    img_list_.resize(num, std::make_unique<IMAGE>());
    for (int i = 0; i < num; i++) {
        std::string path_i = path + std::to_string(i) + ".png";
        loadimage_safe(img_list_[i].get(), path_i.c_str());
    }
}

void Atlas::LoadfromAtlas(Atlas& atlas, bool flip) {
    img_list_.clear();
    img_list_.resize(atlas.size(), std::make_unique<IMAGE>());
    for (int i = 0; i < atlas.size(); i++) {
        if (flip) flipimage(atlas.GetImage(i), img_list_[i].get());
        else *(img_list_[i]) = *(atlas.GetImage(i));
    }
}