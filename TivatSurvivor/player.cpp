...
Player::Player(Position pos, double speed) : 
    position(pos), 
    speed(speed)
{
...
void Player::Move(int dir_x, int dir_y, double delta) {
    //调整角色动画方向
...
void Player::Init(RectArea limit) {
    limit_area = {
    ...
    std::vector<AnimationConfig> animation_config = {
        { "img/player_left_", FRAME_WIDTH, FRAME_HEIGHT, FRAME_NUM, FRAME_INTERVAL },
        { "img/player_right_", FRAME_WIDTH, FRAME_HEIGHT, FRAME_NUM, FRAME_INTERVAL }
    };
    animation_atlas = std::make_shared<Atlas>(animation_config);

    loadimage_safe(&img_shadow, _T("img/shadow_player.png"));
}